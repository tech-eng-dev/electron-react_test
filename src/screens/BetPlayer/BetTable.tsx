...
const ColumnNames = [
    ...
    { id: 'createdAt', algin: 'left', width: 80, label: 'Bet Time' },
    ...
    { id: 'placedOdds', algin: 'left', width: 140, label: 'Placed Odds' },
                ...
                <Table className={css(styles.table)}>
                    ...
                    <TableBody>
                        {displayBets.map((bet, index: number) => {
                            ...
                            if (bet.won === 1) resultColor = 'rgba(100,255,100,1)';
                            else if (bet.won === -1) resultColor = 'rgba(255,100,100,1)';
                            else if (bet.won === 2) resultColor = '#9e9e9e';

                            return (
                                <TableRow className={css(styles.tr)} key={bet.id || (bet.uuid + '_' + bet.username + "_" + bet.betTime)}>
                                    ...
                                    <TableCell align={ColumnNames[1].algin as any} className={css(styles.tableCell)}>{moment(bet.betTime).format('MM/DD HH:mm')}</TableCell>

                                    <TableCell align={ColumnNames[2].algin as any} className={css(styles.tableCell)}>
                                        {bet.username}
                                    </TableCell>

                                    <TableCell align={ColumnNames[3].algin as any} className={css(styles.tableCell)}>{`${bet.bookmaker}`}</TableCell>
                                    <TableCell align={ColumnNames[4].algin as any} className={css(styles.tableCell)}>{bet.sport}</TableCell>
                                    <TableCell align={ColumnNames[5].algin as any} className={css(styles.tableCell)}>{bet.eventName}</TableCell>
                                    <TableCell align={ColumnNames[6].algin as any} className={css(styles.tableCell)}>{bet.marketType + (bet.handicap != null ? ` (${util.toPrintableNumber(bet.handicap)})` : '')}</TableCell>
                                    <TableCell align={ColumnNames[7].algin as any} className={css(styles.tableCell)}>{bet.selectionName}</TableCell>
                                    <TableCell align={ColumnNames[8].algin as any} className={css(styles.tableCell)}>{util.financial(bet.placedOdds || 0).toFixed(2)}</TableCell>
                                    <TableCell align={ColumnNames[9].algin as any} className={css(styles.tableCell)}>{bet.placedStake ? bet.placedStake : null}</TableCell>
                                    <TableCell align={ColumnNames[10].algin as any} className={css(styles.tableCell)}>{Number.isNaN(profit) ? 'Not Yet' : profit.toFixed(2)}</TableCell>
                                </TableRow>
                            );
                        })}
                    </TableBody>
